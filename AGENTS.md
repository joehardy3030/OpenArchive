...
- Debug builds use Xcode's debug dylib: `Chateau.app/Chateau` is a small stub and the compiled code — strings, symbols — lives in `Chateau.app/Chateau.debug.dylib`. Check that file when verifying which code a build contains.

## Testing

`BreazeTests/BreazeTests.swift` is a real unit test suite run via Xcode (Cmd+U) or `xcodebuild test -workspace Breaze.xcworkspace -scheme Breaze -destination 'platform=iOS Simulator,name=<device>'`. Every behavior fix in this codebase's recent history is pinned by a test, often built from the exact archive.org payload that exposed it. Coverage:
...
- **Playback engine / CarPlay handoff**, driven against the shared engine with a dummy `AVQueuePlayer`: one remote play or toggle swallowed during the 30s connect grace (including delayed cold-start commands), an immediate second press honored, explicit play clearing suppression, playing/buffering sessions preserved on connect, empty queues not spending the swallow, disconnect pausing and saving, route loss pausing even without a scene callback, and interruption resume canceled by disconnect or an explicit pause. Also covers `pause(persist:)`, saves skipped with nothing loaded, guarded paused restore, queued recovery honoring disconnect and tape changes, delayed restore leaving a newly selected tape alone, and `getCurrentTrackIndex`. Async recovery/restore tests use a temporary silent audio file, including verification of the saved seek position. Every engine test resets the singleton in a `defer`.
...
`ArchiveRootView` also owns the global **MiniPlayerBar** (shown above the tab bar whenever a show is loaded — a real keyboard simply covers it) and presents **FullPlayerView** as a sheet. The entire root shell uses `.ignoresSafeArea(.keyboard)`: iOS can leave a phantom keyboard inset stuck after share sheets or tab switches, which squeezed content to the top half of the screen and, when the old show/hide boolean got stuck, hid the mini player entirely. Never gate layout on keyboard show/hide notifications — they fire spuriously. The Search form does its own avoidance via `KeyboardObserver`, which derives overlap from keyboard *frame geometry* (an off-screen frame computes to zero), applied as a bottom `safeAreaInset` minus the tab-bar inset. A `miniPlayerInset` EnvironmentKey pads scrolling lists so content isn't hidden behind the mini player.
...
**Creator-based search:** Phish and Jerry Garcia are creator-based defaults (`creator:"X"` instead of `collection:(X)`); extra runtime-detected creator-based identifiers live in UserDefaults key `creatorBasedCollectionsExtra`, and Taper's Section artists are added creator-based too, which finds their tapes across all archive.org collections. Creator names are percent-encoded in hand-built query URLs via `ArchiveAPI.encodeQueryValue` (names can contain spaces and `&`). Important: archive.org's `creator:"X"` is a **phrase match, not exact equality** — `creator:"Jerry Garcia"` also matches "Jerry Garcia Band" and "Jerry Garcia Acoustic Band" (desired) as well as post-1995 tribute acts (excluded via the curated 1963–1995 year range). Display-level, `ShowMetadata.displayCreator` refines the generic index creator via the identifier's taper band-code token (`jgb`→Jerry Garcia Band, `jgjk`→Jerry Garcia & John Kahn, `oaitw`→Old & In the Way, etc. — table in `ShowMetadataModel.bandCodes`, verified against catalog-wide token frequencies) and feeds every band-name render: rows, detail, both players, and Now Playing. Grouping under the Jerry Garcia band entry is query-level and unaffected.
...
- Failure recovery: when a **local** item fails (corrupt/truncated file), the path is added to `pathsToForceStream` (per-show, in-session), the queue is rebuilt from the current track using the streaming URL for the bad file (playback resumes only if it was playing — a paused session just gets its queue repaired; deferred recovery checks the current queue and playback intent so a later disconnect or tape selection wins), and a silent re-download repairs the disk copy. When a **streaming** item fails, up to 3 consecutive failures are skipped before going idle.
- System integration: `MPRemoteCommandCenter` (play/pause/toggle/next/previous — the play and toggle decisions live in `handleRemotePlay()` / `handleRemoteTogglePlayPause()`, outside the command-target closures, so they're unit-testable), `MPNowPlayingInfoCenter` with downloaded cover art (`setArtworkURL`, falls back to app icon), and audio-session interruption handling with auto-resume (only if it was playing, and never for a `routeDisconnected` interruption — see CarPlay). `isActivelyPlaying` and `remotePlaySuppressed` are the read-only state accessors.
...
- A blue checkmark indicates a fully downloaded show consistently across the show detail page, Downloads rows, and Favorites rows. Per-track checkmarks are reconciled with the disk on every detail open (`ShowDetailViewModel.reconcileDestinations`, off-main) — the repair flow and fresh fetches don't carry `destination` fields, so disk is the source of truth for both levels. Tapping it offers to delete the downloaded files (via `NetworkUtility.deleteDownloadedShow`), after which the show can be downloaded fresh.
...
- The CarPlay UI is a single "My Tapes" `CPListTemplate` of fully downloaded shows (incomplete ones filtered out), paginated 10 per page with See more / Show previous rows; no section header (it would duplicate the title in the small display). Tapping the already-loaded show resumes rather than restarting, opening Now Playing via the shared `CPInterfaceController.pushNowPlaying()`. There is deliberately **no "Continue Listening" row**: CarPlay's own Now Playing button (top right) already represents the loaded track, playing or paused. Band names come from `ShowMetadata.displayBandName`, the same helper the phone uses.
...
- **Phone ↔ CarPlay handoff** is inherent: both drive the one `AudioPlayerArchive` in one process. Across launches it rests on `PlaybackState`: `CarPlayTemplateManager` runs `PlayerViewModel.restorePlaybackIfAvailable()` right after installing the root template (a CarPlay-only launch never shows the phone window; after the template so a cold car launch paints first), and that method is guarded (`showMetadataModel == nil`) so whichever side connects first restores and the other never clobbers live playback. The restore publishes Now Playing info at rate 0 so the paused session shows up in CarPlay without starting. Its delayed seek and completion are bound to the restored queue, and publication respects an explicit play during preparation.
- **Play/pause across plug-in/out:** `CarPlaySceneDelegate.didConnect` calls `AudioPlayerArchive.suppressAutoResumeOnConnect()`. When paused, the engine swallows exactly the next remote play or toggle within 30s; the allowance catches slow cold starts. An immediate second press always works, and explicit `play()` from the phone or My Tapes clears suppression. There is no burst suppression and no Now Playing visibility shortcut: CarPlay can restore that screen automatically, so its presence never proves a person pressed play. Remote command events do not identify automatic versus human intent, so repeated automatic commands are indistinguishable from the second human press; commands after the grace expires are honored. Playing sessions (including buffering at rate zero) continue on connect. Commands with no current item fail without spending the swallow. A swallowed play republishes Now Playing at rate 0. Connecting cancels any pending interruption resume. Both the CarPlay-specific `templateApplicationScene(_:didDisconnectInterfaceController:)` (note the asymmetric Swift name — a `didDisconnect:` spelling compiles but is never called; the compiler only emits a "nearly matches" warning) and `sceneDidDisconnect` callbacks share an idempotent cleanup path through `handleCarPlayDisconnect()`, which pauses, saves the stop point, cancels interruption resume even when already paused, and clears the startup swallow for phone controls. Audio-session `oldDeviceUnavailable` route changes also pause and cancel resume, even before a scene callback. A `routeDisconnected` interruption never arms resume; a regular call resumes only when it interrupted active playback, the system permits it, and no subsequent pause/disconnect canceled it. Remote pause/toggle use engine intent rather than queue rate so buffering does not defeat pause.
- **Diagnosing in the car:** the engine's CarPlay-relevant events (connect/disconnect, every remote play/toggle with the seconds since connect and whether it was swallowed, interruptions with their reason, restore, every state change) go to the unified log via `AudioPlayerArchive.log` (`Logger`, subsystem `com.carquinez.chateau`, category `playback`, `.notice` level so it persists; values marked `privacy: .public`). `print` output never leaves a phone that isn't attached to Xcode, so don't rely on it. After a drive, pull the phone's log on the Mac (needs root; zsh's `log` builtin shadows the tool, so use the full path) and filter: `sudo /usr/bin/log collect --device-udid <udid> --last 2h --output car.logarchive` then `/usr/bin/log show --archive car.logarchive --predicate 'subsystem == "com.carquinez.chateau"'`; the same archive holds `mediaremoted`'s record of which commands the system delivered. The udid `log collect` wants is the **hardware** UDID (`xcrun devicectl device info details --device <id>` → `hardwareProperties.udid`, the `00008140-…` form), not the CoreDevice UUID that `xcrun devicectl list devices` prints — that one gives "Device not configured"; `--device-name "<name>"` also works. The phone should be on the USB cable for the collect. `xcrun devicectl device info apps --device <id>` gives the installed build number (= git commit count at build time).
...
- Playback state is saved on app background/termination, on every track change, on user/system pauses (`pause(persist:)` — pre-rebuild stops pass `false`, since the queue is replaced moments later and the track-change save captures the real state), and every 30s during playback (`AudioPlayerArchive.savePlaybackStateThrottled`, on the engine's own tick, JSON-encoded off the main thread — the blob includes the full files array). Restored on next launch (paused, seeked to the saved position) from either the phone window or the CarPlay scene, whichever appears first. Stored as JSON in UserDefaults key `lastPlaybackState`; discarded when older than 7 days (`PlaybackState.isStale`).
