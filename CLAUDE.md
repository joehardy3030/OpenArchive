...
- **Playback engine / CarPlay handoff**, driven against the shared engine with a dummy `AVQueuePlayer` (the item's file doesn't exist — these exercise state, not audio): the remote-play connect grace (the first play swallowed when idle and a second one honored, honored once expired, no-op when already playing, cleared by an explicit `play()` or by disconnect, mirrored by toggle, and the cold-start sequence where an early play with nothing loaded fails without spending the swallow), `pause(persist:)` semantics, saves skipped when nothing is loaded, the restore guard (no-op with a show loaded; loads the saved session paused when idle), and `getCurrentTrackIndex` (plain, nested, and space-containing filenames). Every engine test resets the singleton in a `defer`.
...
- **Play/pause across plug-in/out:** car head units send a remote `play` on connect (resuming the last audio source). `CarPlaySceneDelegate.didConnect` calls `AudioPlayerArchive.suppressAutoResumeOnConnect()`, which, *when we weren't already playing*, arms the engine to swallow the **next** remote play that arrives within a 20s window — a paused session stays paused on plug-in, a playing one keeps playing, and any explicit `play()` from our own UI clears it. Exactly one play is swallowed, not every play in a short window: a cold car start delivers the head unit's command many seconds after the scene connects (after the restore has published Now Playing), which a fixed 3s window missed, while a person who taps play twice always gets through. A play that arrives before anything is loaded fails without spending the swallow, so the head unit's later retry is still caught. `sceneDidDisconnect` pauses (persisting the stop point for the phone to pick up) and cancels any unspent swallow so the phone's next lock-screen play isn't eaten. The engine logs the elapsed time whenever it swallows one ("swallowed the head unit's auto-play …s after CarPlay connect") — the first thing to check in Console if a car still misbehaves.
