...
- **Play/pause across plug-in/out:** the app follows the platform convention — a car head unit's connect-time remote `play` resumes the last session, exactly as Apple Podcasts and Music do, even when that session was paused. There is deliberately **no suppression window**: an earlier version tried to keep a paused session paused across plug-in, and it cannot be done correctly. `MPRemoteCommandEvent` carries nothing that distinguishes a head unit's automatic play from a person pressing play, and every proxy for the distinction failed in the car — a fixed grace window missed cold starts that deliver the command 5s+ after the scene connects; a one-shot swallow ate a real first press on head units that send nothing; and Now Playing screen visibility proved nothing because CarPlay restores that screen itself on a re-plug. Don't reintroduce it. What remains is the other direction, which is unambiguous: `handleCarPlayDisconnect()` (shared by `templateApplicationScene(_:didDisconnectInterfaceController:)` — note the asymmetric Swift name, a `didDisconnect:` spelling compiles but is never called, with only a "nearly matches" warning — and `sceneDidDisconnect`, guarded so the two callbacks are idempotent) pauses and saves the stop point for the phone to pick up. Audio-session `oldDeviceUnavailable` route changes pause too, covering the case where the route dies before any scene callback, and never resume onto the phone speaker. A `routeDisconnected` interruption never arms auto-resume; a regular call resumes only when it interrupted active playback, the system permits it, and no later pause or disconnect canceled it. Remote pause/toggle test engine intent (`isActivelyPlaying`) rather than queue rate, so buffering doesn't defeat a pause.
